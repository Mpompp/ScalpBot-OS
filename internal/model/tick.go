...
func NormalizeSymbol(sym string) string {
	...
	for _, suf := range []string{".PRO", ".M", ".R", "M"} {
	...
	// The TrimSuffix("C") is safe for broker cent-account suffixes (XAUUSDc, EURUSDc)
	return s
}

// Tick represents a single market tick with bid/ask prices.
...
type Tick struct {
	Symbol      string  // Currency pair, e.g. "EURUSD", "XAUUSD"
...
// - Gold (XAUUSD): 1 pip = $0.01 (100 pips = $1.00 move in gold price)
// - Standard Forex: 1 pip = 0.0001 (multiplier = 10000)
...
func PipMultiplier(symbol string) float64 {
	...
	return 10000.0 // Standard Forex pairs
}
...
func PipValue(symbol string, lots float64) float64 {
	...
	return lots * 10.0 // Standard Forex fallback
}
